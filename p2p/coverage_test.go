...
func TestMapAndMetricHelpers(t *testing.T) {
	...
	require.Equal(uint32(1), mp.PeerMessageTypeSnapshotResponse)
	require.Equal(uint32(1), mp.PeerMessageTypeFullChallenge)
...
func TestBuildAndParseNetworkMessages(t *testing.T) {
	...
	msg, err = parseNetworkMessage(7, buildBatchTransactionChallengeMessage(snapshot.PayloadHash(), cosi, []*common.VersionedTransaction{tx, fullVer}))
	require.Nil(err)
	require.EqualValues(PeerMessageTypeBatchTransactionChallenge, msg.Type)
	require.Len(msg.Transactions, 2)
	require.Equal(fullVer.PayloadHash(), msg.Transactions[1].PayloadHash())
	...
	require.Nil(err)
	require.EqualValues(PeerMessageTypeBatchSnapshotResponse, msg.Type)
	require.Equal(byte(1), msg.Response[0])

	msg, err = parseNetworkMessage(7, buildBatchSnapshotFinalizationMessage(snapshot))
	require.Nil(err)
	require.EqualValues(PeerMessageTypeBatchSnapshotFinalization, msg.Type)
	require.Equal(snapshot.PayloadHash(), msg.Snapshot.PayloadHash())

	msg, err = parseNetworkMessage(7, buildBatchSnapshotFinalizationMessage(batchSnapshot))
	require.Nil(err)
	require.EqualValues(PeerMessageTypeBatchSnapshotFinalization, msg.Type)
	require.Equal(batchSnapshot.PayloadHash(), msg.Snapshot.PayloadHash())
	require.Len(msg.Snapshot.Transactions, 2)
	...
	msg, err = parseNetworkMessage(7, buildBatchFullChallengeMessage(handle, snapshot, &commitment, &challenge, []*common.VersionedTransaction{fullVer}))
	require.Nil(err)
	...
	msg, err = parseNetworkMessage(7, buildBatchFullChallengeMessage(handle, snapshot, &commitment, &challenge, []*common.VersionedTransaction{tx, fullVer}))
	require.Nil(err)
	require.EqualValues(PeerMessageTypeBatchFullChallenge, msg.Type)
	require.Equal(snapshot.PayloadHash(), msg.Snapshot.PayloadHash())
	require.Len(msg.Transactions, 2)
	...
	_, err = parseNetworkMessage(7, []byte{PeerMessageTypeBatchSnapshotResponse, 1})
...
func TestP2PMessageAndPeerEdgeCases(t *testing.T) {
	...
	shortCommitments[0] = PeerMessageTypePreCommitments
	...
	malformedCommitments[0] = PeerMessageTypePreCommitments
	...
	_, err = parseNetworkMessage(7, bytes.Repeat([]byte{PeerMessageTypeBatchSnapshotAnnouncement}, 10))
	require.ErrorContains(err, "invalid announcement message size")
	badAnnouncement := append([]byte{PeerMessageTypeBatchSnapshotAnnouncement}, bytes.Repeat([]byte{1}, 100)...)
	...
	full := buildBatchFullChallengeMessage(handle, snapshot, &commitment, &challenge, []*common.VersionedTransaction{fullVer})
	...
	offset := 65 + 4 + len(snapshot.VersionedMarshal()) + 32 + 32 + 1
...
func TestHandlePeerMessageDispatch(t *testing.T) {
	...
	parsed, err := parseNetworkMessage(TransportMessageVersion, buildCommitmentsMessage(handle, []*crypto.Key{&commitment}))
	require.NoError(err)
	...
	parsed, err = parseNetworkMessage(TransportMessageVersion, buildBatchFullChallengeMessage(handle, snap, &commitment, &challenge, []*common.VersionedTransaction{tx}))
	require.NoError(err)
	err = me.handlePeerMessage(peerID, parsed)
	require.Nil(err)
	...
	err = me.handlePeerMessage(peerID, &PeerMessage{
		Type:         PeerMessageTypeBatchSnapshotResponse,
		SnapshotHash: snap.PayloadHash(),
		Response:     response,
	})
	require.Nil(err)
	...
	err = me.handlePeerMessage(peerID, &PeerMessage{
		Type:     PeerMessageTypeBatchSnapshotFinalization,
		Snapshot: snap,
	})
	require.Nil(err)
	...
	for _, payload := range [][]byte{
		buildBatchFullChallengeMessage(handle, batchSnap, &commitment, &challenge, []*common.VersionedTransaction{tx}),
...
func TestSendMessageHelpers(t *testing.T) {
	...
	batchSnapshot := p2pTestBatchSnapshot(true)
	batchSnapshot.Hash = batchSnapshot.PayloadHash()
	tx := p2pTestTransaction()
	fullTx := common.NewTransactionV5(common.XINAssetId)
	...
	err = me.SendTransactionChallengeMessage(target, snapshot, cosi, []*common.VersionedTransaction{tx})
	require.Nil(err)
	require.EqualValues(PeerMessageTypeBatchTransactionChallenge, (<-neighbor.normalRing).data[0])

	err = me.SendTransactionChallengeMessage(target, batchSnapshot, cosi, []*common.VersionedTransaction{tx, fullVer})
	require.Nil(err)
	require.EqualValues(PeerMessageTypeBatchTransactionChallenge, (<-neighbor.normalRing).data[0])
	...
	require.Nil(err)
	require.EqualValues(PeerMessageTypeBatchSnapshotResponse, (<-neighbor.normalRing).data[0])
	...
	err = me.SendSnapshotFinalizationMessage(target, batchSnapshot)
	require.Nil(err)
	require.EqualValues(PeerMessageTypeBatchSnapshotFinalization, (<-neighbor.normalRing).data[0])
...
func (h *p2pStubHandle) CosiQueueExternalAnnouncement(_ crypto.Hash, s *common.Snapshot, _ *crypto.Key) error {
	h.announcement = s
	return nil
}

func (h *p2pStubHandle) CosiAggregateSelfCommitments(_ crypto.Hash, _ crypto.Hash, _ *crypto.Key, wantTxs []crypto.Hash) error {
	h.wantTxs = wantTxs
	return nil
}
...
func (h *p2pStubHandle) CosiQueueExternalFullChallenge(_ crypto.Hash, snapshot *common.Snapshot, _ *crypto.Key, _ *crypto.Key, _ *crypto.CosiSignature, txs []*common.VersionedTransaction) error {
...
func (h *p2pStubHandle) CosiQueueExternalPreCommitments(_ crypto.Hash, commitments []*crypto.Key) error {
...
func p2pTestBatchSnapshot(withSignature bool) *common.Snapshot {
	s := p2pTestSnapshot(withSignature)
	s.AddTransaction(crypto.Blake3Hash([]byte("snapshot-transaction-batch")))
	return s
}
