...
func TestCommonDataEncoding(t *testing.T) {
	...
	require.Equal(msgpackMarshalPanic(res), msgpackMarshalPanic(mint))
}
...
func TestCommonDataEncodingLegacy(t *testing.T) {
	...
	enc := mint.Marshal()
	require.Equal("777700010001000000000000007b000412b9af98eea889c227076f8c62106b59a478e043c0030392f3be0f5d714ed27953cb2668", hex.EncodeToString(enc))
	enc = mint.CompressMarshal()
	require.Equal("0000000028b52ffd0300c118533ca10100777700010001000000000000007b000412b9af98eea889c227076f8c62106b59a478e043c0030392f3be0f5d714ed27953cb2668", hex.EncodeToString(enc))
	res, err := DecompressUnmarshalMintDistribution(enc)
	require.Nil(err)
	require.Equal(mintGroupKernelNodeLegacy, res.Group)
	require.Equal(uint64(123), res.Batch)
	require.Equal("3.14159000", res.Amount.String())
	require.Equal("eea889c227076f8c62106b59a478e043c0030392f3be0f5d714ed27953cb2668", res.Transaction.String())
	require.Equal(msgpackMarshalPanic(res), msgpackMarshalPanic(mint))
