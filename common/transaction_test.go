...
func TestTransactionReferences(t *testing.T) {
	...
	assert.Equal("d9c6168b677ec72e351f05f7acc4c34fe8d9d606fa8645241b7407918a649c68", ver.PayloadHash().String())
	ver.AddInput(genesisHash, 0)
	ver.resetCache()
	assert.Equal("8f5b8ac9de4733db496b5fa6305cada963a2da703ecc2c101ee579c001ac12cb", ver.PayloadHash().String())
	ver.AddInput(genesisHash, 1)
	ver.resetCache()
	assert.Equal("1f4e5c9fec2eba9a4af1dd6917eba0bb7e26201002d0a75df11b9d605e2fe6b4", ver.PayloadHash().String())
	...
	ver.resetCache()
	assert.Equal("3c4c47783aae1a492dbcfec32c2f8a27714367b92a89e7c792144cca913204dc", ver.PayloadHash().String())
	ver.AddScriptOutput(accounts, script, NewInteger(10000), bytes.Repeat([]byte{1}, 64))
	ver.resetCache()
	assert.Equal("4637bb62c48a595e50468a099aa221a2f82f7c8d5be1ae872fed93d8665c07d6", ver.PayloadHash().String())

	pm := ver.Marshal()
	assert.Equal(740, len(pm))
