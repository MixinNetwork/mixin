...
func getInfo(store storage.Store, node *kernel.Node) (map[string]any, error) {
	cs, _ := node.ReadLastConsensusSnapshotWithHack()
	info := map[string]any{
	...
	if block != nil {
		seq["height"] = block.Number
		seq["sequence"] = block.Sequence
		seq["hash"] = block.PayloadHash()
		seq["timestamp"] = time.Unix(0, int64(block.Timestamp))
	}
	info["sequencer"] = seq
	return info, nil
}
