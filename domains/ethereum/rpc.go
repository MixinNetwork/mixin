...
// nohup parity --tracing on --cache-size 4096 --rpcapi eth,traces,parity --rpcaddr 0.0.0.0 --rpcport 8545 2>&1 > ~/.ethereum/parity.log &

const (
	ethereumMinimumHeight       = 1000000
	ethereumHost                = "ethereum-full-node:8545"
...
func (chain *RPC) GetTransactionResult(transactionHash string) (*external.Transaction, error) {
	...
	receiptStatus, err := ethereumParseNumber(receipt.Status)
	if err != nil {
		return nil, err
	}
	if receiptStatus.Cmp(big.NewInt(1)) != 0 {
		txReceipt = external.TransactionReceiptFailed
	}
...
func (chain *RPC) parseInternalTransaction(ctx context.Context, receipt *EthereumReceiptResult, item *EthereumTraceResult, height int64, index int) (*external.Transaction, error) {
	...
	if asset.Precision < 0 || asset.Precision > 32 {
		return nil, nil
	}
...
func (chain *RPC) getTransferEvent(receipt *EthereumReceiptResult, token *EthereumToken, input string) (number.Decimal, string, string, error) {
	...
	if consumedLog > 0 {
		receipt.Logs[consumedLog] = receipt.Logs[len(receipt.Logs)-1]
		receipt.Logs = receipt.Logs[:len(receipt.Logs)-1]
	}
...
func (chain *RPC) getTokenUpperSymbolAndName(ctx context.Context, address string) (string, string, error) {
	...
	result, err := chain.ethereumCall(address, erc20SYMBOLSignature, 0x100000)
	...
	result, err = chain.ethereumCall(address, erc20NAMESignature, 0x100000)
	if err != nil || len(result) < 66 {
		return "", "", err
	}
	resultBytes, err = hex.DecodeString(result[2:])
...
func (chain *RPC) getTokenSymbolAndNameAndDecimals(ctx context.Context, address string) (string, string, int64, error) {
	...
	result, err := chain.ethereumCall(address, erc20DecimalsSignature, 0x100000)
	if err != nil || len(result) != 66 {
		return "", "", 0, err
	}
	...
	if bigDecimals.Cmp(big.NewInt(32)) > 0 {
		return "", "", 0, err
	}
...
func (chain *RPC) checkERC20Token(ctx context.Context, address string) (*EthereumToken, error) {
	...
	if len(symbol) > 16 || len(name) > 128 {
		return nil, nil
	}
	if !utf8.ValidString(symbol) || !utf8.ValidString(name) {
		return nil, nil
	}
...
func (chain *RPC) ethereumCall(to, data string, gasLimit int64) (string, error) {
	params := map[string]interface{}{"to": to, "data": data, "gas": fmt.Sprintf("0x%x", gasLimit)}
