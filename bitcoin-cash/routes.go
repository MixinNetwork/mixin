...
func getAsset(w http.ResponseWriter, r *http.Request, params map[string]string) {
	if params["id"] == external.BitcoinCashChainId {
		views.RenderDataResponse(w, r, map[string]interface{}{
			"chain_id":        external.BitcoinCashChainId,
			"asset_id":        external.BitcoinCashChainId,
			"chain_asset_key": external.BitcoinCashChainId,
			"symbol":          "BCH",
			"name":            "Bitcoin Cash",
			"precision":       8,
		})
