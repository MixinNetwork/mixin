...
const (
	...
	bitcoinHost                 = "bitcoin-cash-full-node:8332"
...
func NewRPC() (*RPC, error) {
	chain := &RPC{
		...
		id:       external.BitcoinCashChainId,
	}
...
func (chain *RPC) GetBlockByHash(ctx context.Context, blockHash string) (*external.Block, error) {
	...
	for _, txId := range resp.Result.Tx {
		...
		for _, out := range tx.Vout {
			...
			receiver, err := chain.convertCashAddr(out.ScriptPubKey.Addresses[0])
			if err != nil {
				return nil, err
			}
...
func (chain *RPC) convertCashAddr(addr string) (string, error) {
	if strings.HasPrefix(addr, "bitcoincash:") {
		data, prefix, typ, err := bchutil.CheckDecodeCashAddress(addr)
		if err != nil {
			return "", err
		}
		if prefix != "bitcoincash" || typ != 0 {
			return "", fmt.Errorf("malformed bitcoincash addr %s %d", prefix, typ)
		}
		addr = base58.CheckEncode(data, byte(typ))
	}
	btcAddress, err := btcutil.DecodeAddress(addr, &chaincfg.MainNetParams)
	if err != nil {
		return "", err
	}
	if btcAddress.String() != addr {
		return "", fmt.Errorf("Bitcoin Cash convert mismatch %s", addr)
	}
