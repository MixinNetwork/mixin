...
time. The [range fixtures](testdata/README.md) contain proofs generated by the
pinned Monero implementation for valid boundary amounts and out-of-range
amounts `2^64` and `2^248`, including padded and maximum-size aggregates.
Tests check the full-width commitment amounts and require rejection by the
verification equation, both individually and in mixed batches.

The main test also pins SHA-256 known-answer digests for deterministic proofs
...
and verifies in both directions. It also checks that both implementations
agree on the Monero range fixtures, individually and in batches:
